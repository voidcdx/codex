...
async function _doUpdateModdedStats() {
  ...
  try {
    const resp = await fetch('/api/modded-weapon', {
      ...
      body: JSON.stringify({ weapon: weapon.name, mods, attack: selectedAttack, riven: getRivenSpec(), galvanized_stacks: parseInt(document.getElementById('galv-stacks').value, 10) || 0, cold_stacks: parseInt(document.getElementById('cold-stacks').value, 10) || 0, combo_counter: (Math.max(1, parseInt(document.getElementById('combo-counter')?.value, 10) || 1) - 1) * 10, arcanes: getActiveArcanes(), ...getBonusElement() }),
    });
    if (!resp.ok) return;
